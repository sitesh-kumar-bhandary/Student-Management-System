    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            if (requestTokenHeader == null ||
                ...
                log.debug(
                        "No JWT token found in request for URI: {}",
            ...
            log.debug(
                    "JWT token detected in request for URI: {}",
                    request.getRequestURI());
            ...
            if (username != null &&
                ...
                log.debug(
                        "Extracted username '{}' from JWT for URI: {}",
                        username,
                        request.getRequestURI());
                ...
                if (authUtils.isTokenValid(token, userDetails)) {
                    ...
                    log.info(
                            "JWT authentication successful for user '{}' on URI: {}",
                            username,
                            request.getRequestURI());
                } else {
                    log.warn(
                            "Invalid JWT token for user '{}' on URI: {}",
                            username,
                            request.getRequestURI());
                }
            }
